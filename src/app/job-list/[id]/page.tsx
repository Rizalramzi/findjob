...
// Interface untuk tipe data PageProps
interface PageProps {
  params: {
    id: string;
  };
...
async function fetchServiceDetail(id: number) {
  try {
    const response = await fetch(
      'https://tech-class.datacore.machinevision.global/items/Service_ramzi', 
      {
        method: 'GET',
        headers: {
    ...
    const serviceData = data.data[id - 1]; // Mengambil data berdasarkan ID
    ...
    return {
      ...
      skills: ['HTML', 'CSS', 'Tailwind'], // Skills statis untuk sementara
    };
  } catch (error) {
...
export async function generateStaticParams() {
  ...
  // Mapping data untuk membuat ID rute
